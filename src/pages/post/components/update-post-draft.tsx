...
import { Button, Card, Flex, Form, Image, Input, Modal, Select, Space, Tooltip, Upload } from 'antd';
...
import { useUpdateDraftToPost, useUpdatePostDraft } from '@/hooks/mutate/post/use-update-post';
...
export const UpdatePostDraft: FC<UpdatePostProps> = ({ onCancel }) => {
    ...
    const { data: topics, isLoading: isLoadingTopics } = useTopicsListing({
        params: { ...initialParams, categoryId: searchParams.get('category') },
    });
    ...
    const onPost = () => {
        ...
        form.validateFields().then(() => {
            ...
            put(`/post/update/draft/to-post/${id}`, body)
                .then(resp => {
                    if (resp.status === 200) {
                        success('Post updated successfully!');
                        onCancel && onCancel();
            ...
            <Card>
                <Flex vertical gap={10}>
                    ...
                    <Flex align="center" justify="space-between">
                        ...
                        <Space>
                            <Button onClick={onPost} loading={loading}>
                                Post
                            </Button>
