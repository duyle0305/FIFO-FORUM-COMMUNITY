...
const AdminTopicPage = () => {
    ...
    const [params, setParams] = useState<TopicListingParams>({ page: DEFAULT_PAGE, perPage: DEFAULT_PAGE_SIZE });
    const [search, setSearch] = useState('');

    const searchDebounce = useDebounce(search, 500);

    useEffect(() => {
        setParams(prev => ({ ...prev, name: searchDebounce, page: DEFAULT_PAGE }));
    }, [searchDebounce]);

    const { data, isFetching } = useTopicsListing({ params: { ...params } });
