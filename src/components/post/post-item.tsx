...
export const PostItem: FC<PostItemProps> = ({
    ...
    const handleBookmark = (id: string, isBookmarked: boolean) => {
        Modal.confirm({
            title: 'Confirm',
            content: `Do you want to ${isBookmarked ? 'remove' : 'add'} the post from the Bookmarks list?`,
            onOk: () => {
                toggleBookmark(
                    { postId: id },
                    {
    ...
    const isAllowShowOnlyDeleteAction = accountInfo?.role?.name === 'STAFF';
    ...
    const handleUpdateStatus = async (status: string) => {
        ...
        try {
            const endpoint = `/post/update/${id ? id : postId}/${url}`;
    ...
    return (
        data && (
            <Card style={{ cursor: 'pointer', marginBottom: 16, color: 'white' }}>
                <Flex vertical gap={8}>
                            ...
                            {showActions && isAllowShowActions && (
                                <Dropdown
                                    menu={{
                                        ...
                                        // ],
                                        items: [
                                            ...(isAllowShowOnlyDeleteAction
                                                ? [
                                                      {
                                                          key: '3',
                                                          icon: <DeleteOutlined />,
                                                          label: (
                                                              <span>{isDraft ? 'Delete draft' : 'Delete post'}</span>
                                                          ),
                                                          onClick: (e: any) => {
                                                              e.domEvent.stopPropagation();
                                                              handleDelete();
                    ...
                    >
                        ...
                        <ToggleTruncateTextTypography content={content} maxLength={600} />
                        ...
                        <Spin spinning={isLoadingDownload} size="small">
                            ...
                            >
                                {data?.postFileList?.length > 0 &&
                                    data?.postFileList?.map(post => getFileNameFromUrl(post.url)) && (
                                        <FileZipOutlined />
                                    )}
                                <Typography.Link
                                    onClick={e => {
                        ...
                        <Flex gap={32} vertical onClick={e => e.stopPropagation()}>
                                ...
                                <Flex justify="end" gap={20}>
                                    ...
                                    {bookmarks?.find(bookmark => bookmark?.postId === data?.postId) ? (
                                        <IconButton
                                            ...
                                            children="Bookmark"
                                            onClick={() => handleBookmark(data?.postId, true)}
                                        ...
                                        <IconButton
                                            icon={<FaRegBookmark />}
                                            children="Bookmark"
                                            onClick={() => handleBookmark(data?.postId, false)}
                                            disabled={isPendingToggleBookmark}
                                        />
                                    )}
                                    {isAllowShowReport && (
                            ...
                            {isShowComment && !hideComment && (
                                <PostComment postId={data?.postId} isShown={isShowComment} />
                            )}
                        </Flex>
                    </Col>
                </Flex>
